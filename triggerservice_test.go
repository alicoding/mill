...
// fakeStore is an in-memory settings.Store, so persist()/loadPersistedHotkeys()
// can be tested without a real file on disk. Real OS hotkey registration
// (internal/adapters/hotkey) can't be exercised in headless/CI
// environments (see docs/SPEC.md §1.3) -- these tests cover the
// persistence plumbing and the conflict-rejection rule in isolation
// instead, by manipulating s.hkRaw directly and asserting AssignHotkey
// rejects a conflict *before* it ever reaches the real OS call.
...
func TestPersistHotkeys_WritesBindingsAsJSON(t *testing.T) {
	...
	raw, ok := store.data[triggerHotkeyBindingsKey].(string)
	if !ok {
		t.Fatalf("store[%q] = %#v, want a JSON string", triggerHotkeyBindingsKey, store.data[triggerHotkeyBindingsKey])
	}
...
func TestLoadPersistedHotkeys_MalformedJSON_LogsAndReturns(t *testing.T) {
	store := newFakeStore()
	store.data[triggerHotkeyBindingsKey] = "not valid json"
	comp := NewCompositionService(store)
