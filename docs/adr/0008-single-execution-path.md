...
- **Oscilar** (the reference no-code decisioning platform named
...
- Oscilar's test-run payload is **auto-generated from the workflow's own
...
- **Keep the plain path for canvas authoring speed, add DBOS only for
  "real" runs.** Rejected on the evidence gathered: n8n and Oscilar both
