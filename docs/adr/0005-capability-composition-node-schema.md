...
whether recipes are user-authored on a canvas from day one or config-first
...
- Anti-inner-platform / anti-proliferation (§0): this project has already
  ...
  investment with nothing yet to justify it — zero multi-node recipes
  exist today.
...
- §3.2's already-locked type-vs-instance split: a node's *type* (schema,
  config surface) is defined once and reused across many recipe
  *instances* — whatever schema shape is picked must preserve that.
...
- **B1. React Flow canvas first**, matching the reference platform's own
  ...
  canvas library evaluated, no real multi-node recipe to design it
  against, and it repeats the exact "build the platform before the use
  case" failure mode §0 exists to name.
- **B2. Config-first: a data-driven recipe (ordered/DAG'd node instances
  + their config) editable via a form/JSON side panel, canvas deferred.**
...
control-flow nodes (Decision, Value Assignment, Parallel, Child
Workflow) — composed into a recipe that is data (JSON), authored and
...
deferred, not rejected — revisit once 2+ real multi-node recipes exist
...
- Unlocks: `internal/domain/runbook`'s hardcoded switch can evolve into
  ...
  holding node-type registration, recipe data shape, and execution —
  the actual "core domain" CLAUDE.md keeps pointing at.
- A recipe run is expected to map to one DBOS workflow (ADR-0004); each
  node instance's execution to one DBOS step — not decided in detail
  here, left for whoever implements `internal/domain/composition`, same
  as ADR-0004 left `internal/domain/execution`'s exact shape open.
- Not decided here: the exact Go struct shapes for node type/instance/
  recipe; how session identity (§7) maps onto a DBOS workflow ID for a
  running recipe; §4's connector model (a connector likely becomes one
  more MCP-tool-node source, but that's §4's call); the still-separately-
  `OPEN` MCP-host-vs-server tension from §3.1 (irrelevant to this
  decision — control-flow nodes are Mill's own code either way, not an
  agent loop); versioning/draft-live promotion (a real gap this review
  surfaced, big enough for its own decision once a recipe exists to
  version).
- Risk carried forward: deferring the canvas means Mill's UI stays
  ...
  harder to use than expected once real multi-step recipes exist, not
...
- Maintains: the two-node-family split; config-first-before-canvas
  ordering; the DBOS-workflow-per-recipe expectation
- Update triggers: `internal/domain/composition` actually getting
  scaffolded; a second real multi-node recipe existing (the trigger to
