...
- **Step detail (goal 0058): a large three-pane overlay — recorded
  ...
  `UX: PROTOTYPE`.
- **Canvas note block (goal 0055): a free-floating authoring-space
  annotation, n8n's own "sticky note" precedent — deliberately NOT a
  step.** No `Kind`/`NodeTypeID`, no ports, no `Edge`, invisible to
  `ValidateGraph`/`ExecuteWorkflow`/the guardrail gate, and never
  listed in the step palette (`NodePalette.tsx` only ever renders real
  `NodeType`s). Added via the canvas toolbar's "Add note" button
  (`CanvasToolbar.tsx`), rendered as its own React Flow node type
  (`CanvasNoteView.tsx`, registered `'note'` in `rfNodeTypes.ts`) —
  borderless, a muted color from a small fixed palette, no kind chip —
  visually unmistakable from a step card at a glance. Double-click
  enters inline text editing, a handler kept deliberately separate from
  a step card's own double-click (which opens the step-detail
  overlay); resizable via `@xyflow/react`'s own `NodeResizer` (present
  in the installed version, so no hand-rolled resize math), draggable,
  deletable. Persists with the workflow (`composition.Note`,
  `Workflow.Notes`), saved through its own `CompositionService.
  UpdateNotes` RPC (mirrors `UpdateAttributes`'s existing shape for a
  workflow-scoped collection outside Nodes/Edges), and travels through
  export/import as the envelope's ADDITIVE-OPTIONAL `notes` field
  (ADR-0036 decision 2 — no schema major bump). Dirty-tracked through
  the same hot-exit/save path a step edit already uses. `UX: PROTOTYPE`.
...
| **Canvas note block** (goal 0055: free-floating authoring-space annotation, n8n's own "sticky note" precedent) | Document a workflow's own steps directly on the canvas — movable, resizable, editable text, deletable — without being a step: no ports, no execution, invisible to `ValidateGraph`/`ExecuteWorkflow`/the guardrail gate | Node/graph semantics: build (core domain, `composition.Note`). Resize UI: adopt (`@xyflow/react`'s own `NodeResizer`, confirmed present in the installed version) rather than hand-rolling drag-resize math | `LOCKED`, `UX: PROTOTYPE` — `internal/domain/composition/types.go`'s `Note{ID, Text, Position, Size, Color}` (`Color` a small fixed palette, same convergence discipline as this section's own header), `Workflow.Notes`/`WorkflowVersion.Notes`, saved via its own `CompositionService.UpdateNotes` RPC (the same workflow-scoped-collection shape `UpdateAttributes` already established), exported/imported as the envelope's ADDITIVE-OPTIONAL `notes` field (ADR-0036 decision 2 — schema major stays v1). Canvas: a distinct React Flow node TYPE (`CanvasNoteView.tsx`, registered as `'note'` in `rfNodeTypes.ts`) — borderless sticky, no kind chip, no `Handle`s, added via the canvas toolbar's "Add note" button (never the step palette, which only ever renders real `NodeType` entries), double-click to edit inline (a separate handler from the step card's own double-click, which opens the step-detail overlay), draggable/resizable/deletable, dirty-tracked through the same hot-exit/save path a step edit already uses (`canvasScratch.ts`'s `ScratchDraft.notes`, `useCanvasHotExit`). Seeded proof: "Example: Scratch capture" gained a documenting note (`SeedRevision` bumped), reconcile-upgrade tested. `note_test.go` pins the structural exclusion directly: `ValidateGraph`/`ExecuteWorkflow` only ever accept `[]Node`/`[]Edge`, never a `Note` or a whole `Workflow`, so a Note's content (even an id colliding with a real step) cannot influence either |
