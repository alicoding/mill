...
- **Blocked ≠ unsupported — Mill carries the capability superset; the
  ...
  target work machine: MCP is deny-all (Zscaler blocks the port) until
  ...
  distribution, not ideology: Zscaler scans and passes `git clone` of
