...
step to how long the caller should wait. The tool's own hook timeout
must be longer than this — a hook that gives up first never sees the
reply Mill sends.

## The sender reads the response body

Some tools' hook config posts an event and reads the decision straight
from the response, no script in between. For example, a coding agent's
own `type: "http"` hook posts and reads back this way:
...
A hook config that only runs a command reads the same reply from the
curl response instead:
...
content type — is what the tool's hook receives.

## A quiet failure proceeds, at the sender

If nothing answers, if the reply is malformed for what the tool
expects, or if the tool's own hook times out first, most tools treat
that as a non-blocking failure and let the action proceed anyway. A
workflow meant to gate a tool call needs to say so plainly in its own
reply body, in the tool's own schema — Mill never assumes what
"allow" or "deny" look like on the other end.
