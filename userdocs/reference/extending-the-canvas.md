...
- **Configure entities** — when a tool's own artifact needs a
  ...
  `AddListRow`.
- **Input over the face — declared, not written.** A noun's `content`
  declaration (`'static'` or `'interactive'`) is the whole contract for
