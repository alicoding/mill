...
| `settings` | an array of {key, label, description, defaultValue} boolean setting declarations, or omitted entirely | the noun's own user settings (goal 0258): declared here, rendered generically inside its Settings > Extensions row, persisted centrally per extension id + key. defaultValue applies whenever the user has never touched the toggle. Omitted means the row shows no settings block. |
