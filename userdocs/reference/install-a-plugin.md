...
To preview the automatic rename from the deprecated
`contributes.settings` key to `contributes.configuration`, run:

```sh
mill plugin migrate path/to/your-source-plugin
```

The preview prints the RFC 6902 patch and writes nothing. Add `--json` for a
machine-readable plan, or `--apply` to replace `manifest.json` after the full
plugin validates. A manifest containing both keys needs your decision and is
left unchanged. The command refuses installed plugin folders and install
receipts; run it only against the source folder you author.
