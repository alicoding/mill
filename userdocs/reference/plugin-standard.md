...
21. A view or capture with its own UI declares an entry page:
    ...
    from that folder, so ship what it needs beside it. `window.
    acquireMillApi()` is its door back to Mill. A surface that draws
    into Mill's own document instead still works and warns. (checked)
