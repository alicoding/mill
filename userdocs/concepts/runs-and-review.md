...
- **Recent** — the last few runs that finished, with Done, Failed or
  Stopped and how long ago. Click one to open that run on its
  workflow's canvas and see which steps ran.
...
- **⌘⇧↩** runs the workflow and opens it straight away, so a longer
