...
// The focused List grid's search handle (goal 0349 S4 gap), the same
// focus-scoped shape output.find already established
// (outputFocusStore.ts): listGrid.search has no live selection to act
// on the way the bulk commands (listGridCommands.ts) do, only "toggle
// search on whichever grid currently holds focus" -- and a registry
// command lives at module scope with no way to reach a specific
// mounted ListGridGlide's own state directly. A leaf file, never
