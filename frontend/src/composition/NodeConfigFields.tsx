...
export interface NodeConfigFieldsProps {
  ...
  // since Kind never changes on swap.
  sameKindNodeTypes: NodeType[]
  hasWorkflow: boolean
  // Owned by the caller: it's keyed by *workflow* id
  // (TriggerService.ListHotkeys() is a workflow-id-keyed map, not a
  // node-id-keyed one) and manages a live keydown-recording subscription
  // that must survive across a node re-selection.
...
// The generic ConfigField rendering shared by the sidebar Inspector
// (NodeInspector.tsx) and the step-detail overlay's CONFIG pane
// (StepDetailOverlay.tsx, docs/goals/0058) -- extracted so both render
// the EXACT same field components/edit semantics rather than one
