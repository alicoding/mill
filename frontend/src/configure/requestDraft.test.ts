...
// off, and that draftFrom never leaks a secret back into the form
// (write-only, docs/SPEC.md §3.5).
...
describe('authConfigFrom', () => {
  ...
  it('builds only the OAuth1 branch for AuthOAuth1, leaving OAuth2/HMAC null', () => {
    const got = authConfigFrom({ ...EMPTY_DRAFT, authType: AuthType.AuthOAuth1, oauth1ConsumerKey: 'ck', oauth1Token: 'tok' })
    expect(got).toEqual({ OAuth2: null, HMAC: null, OAuth1: { ConsumerKey: 'ck', Token: 'tok' } })
  })
})

describe('joseConfigFrom', () => {
  it('returns null when JOSE is disabled, regardless of what the other fields hold', () => {
    const got = joseConfigFrom({ ...EMPTY_DRAFT, joseEnabled: false, joseRecipientPublicKeyPEM: 'stale leftover PEM', joseDecryptResponse: true })
    expect(got).toBeNull()
  })
  ...
  it('builds a config with Algorithm/ContentEncryption left empty (domain-layer defaults apply) when enabled', () => {
    const got = joseConfigFrom({
      ...
      joseRecipientPublicKeyPEM: '-----BEGIN PUBLIC KEY-----\nabc\n-----END PUBLIC KEY-----',
  ...
  it('carries DecryptResponse: false through when enabled but response decryption is off', () => {
    const got = joseConfigFrom({ ...EMPTY_DRAFT, joseEnabled: true, joseRecipientPublicKeyPEM: 'pem', joseDecryptResponse: false })
    expect(got?.DecryptResponse).toBe(false)
  })
})

describe('draftFrom', () => {
  it('never populates any secret field from the wire response (write-only design)', () => {
    const r = baseRequest({ AuthType: AuthType.AuthBearer })
    const draft = draftFrom(r)
    expect(draft.secret).toBe('')
    expect(draft.oauth1ConsumerSecret).toBe('')
    expect(draft.oauth1TokenSecret).toBe('')
    expect(draft.josePrivateKeyPEM).toBe('')
  })
  ...
  it('maps Auth.OAuth1 non-secret fields through for AuthOAuth1, without the secret', () => {
    const r = baseRequest({ AuthType: AuthType.AuthOAuth1, Auth: { OAuth2: null, HMAC: null, OAuth1: { ConsumerKey: 'ck', Token: 'tok' } } })
    const draft = draftFrom(r)
  ...
  it('maps JOSE through when present, and stays disabled/empty when JOSE is null', () => {
    const withJOSE = draftFrom(baseRequest({
      JOSE: { Enabled: true, Algorithm: 'RSA-OAEP-256', ContentEncryption: 'A256GCM', RecipientPublicKeyPEM: 'pem-data', DecryptResponse: true },
    }))
    expect(withJOSE.joseEnabled).toBe(true)
    expect(withJOSE.joseRecipientPublicKeyPEM).toBe('pem-data')
    ...
    expect(withoutJOSE.joseRecipientPublicKeyPEM).toBe('')
...
describe('draftFrom + authConfigFrom round trip', () => {
  const cases: Array<[AuthType, HTTPRequest['Auth']]> = [
    ...
    [AuthType.AuthOAuth1, { OAuth2: null, HMAC: null, OAuth1: { ConsumerKey: 'ck', Token: 'tok' } }],
  ]
