...
const INDEX = [
  { rel: 'start-here/what-is-mill.md', title: 'What is Mill', note: '', kind: 'tutorial' },
  { rel: 'start-here/install.md', title: 'Install', note: '', kind: 'tutorial' },
...
describe('groupDocsIndex', () => {
  it('buckets entries by kind, not by directory', () => {
    ...
    expect(groups.map((g) => g.id)).toEqual(['tutorial', 'how-to', 'explanation', 'reference', 'agents'])
  ...
  it('keeps the fixed section order whatever order the entries arrive in', () => {
    ...
    expect(groups.map((g) => g.id)).toEqual(['tutorial', 'explanation', 'reference', 'agents'])
  })
...
describe('groupOf / groupTitleKey', () => {
  it('names the agents section for agents/ pages and the kind otherwise', () => {
