...
// straight off that declaration: shape locks, nothing else does. The
// removed set had exactly this membership, so this is a byte-for-byte
// behavioural regression test, not new behaviour.
describe('isLockableArmTool (goal 0181 S3, regression for goal 0199 part D)', () => {
  it('locks only the tools that declare lockable: true', () => {
    const lockableIDs = ATLAS_TOOLS.filter((t) => t.lockable).map((t) => t.id)
    expect(lockableIDs).toEqual(['shape'])
  })

  it('agrees with each armable tool\'s own declared answer', () => {
    for (const tool of ATLAS_TOOLS) {
      if (tool.interaction === 'arm-then-click' || tool.interaction === 'drag-to-draw' || tool.interaction === 'drag-to-erase' || tool.interaction === 'ephemeral-drag') {
