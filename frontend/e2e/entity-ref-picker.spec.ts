...
// http's connectorId field (RefKind: "connector") renders as a Select
// with a "+ Create new connector…" option instead of a raw paste-an-ID
// text box, over real Go bindings (Wails3 server mode), not mocks.
//
// Deletes both the workflow AND the connector it creates -- the
// connector is a real, separately-persisted Configure entity (same
...
test('Selecting an Integration node offers a live connector picker with inline quick-create', async ({ page }) => {
  await page.goto('/')
  ...
  await expect(inspector).toContainText('Connector ID')

  const picker = inspector.getByTestId('entity-ref-field')
  await expect(picker).toBeVisible()
  await picker.selectOption({ label: '+ Create new connector…' })

  const dialog = page.getByRole('dialog')
  await expect(dialog).toBeVisible()
  await dialog.getByLabel('Label').fill('E2E picker connector')
  await dialog.getByLabel('Base URL').fill('https://api.example.com')
  await dialog.getByRole('button', { name: 'Create' }).click()
  await expect(dialog).toHaveCount(0)

  // The picker auto-selected the newly created connector -- not left on
  // "Select a connector…" or reverted to empty.
  await expect(picker).not.toHaveValue('')
  await expect(picker.locator('option:checked')).toHaveText('E2E picker connector')
  ...
  // Cleanup: the workflow, then the connector it created.
  ...
  await connectorRow(page, 'E2E picker connector').getByRole('button', { name: 'Delete E2E picker connector' }).click()
  await expect(connectorRow(page, 'E2E picker connector')).toHaveCount(0)
})
