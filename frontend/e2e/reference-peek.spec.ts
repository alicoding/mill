...
test('a list reference peeks at its columns and Open lands on that list\'s editor in Configure', async ({ page }) => {
  ...
  await panel.getByTestId('entity-ref-field').first().selectOption({ label: 'Example: Country codes' })
  const peek = panel.getByTestId('entity-ref-peek').first()
  ...
  await expect(page.getByTestId('list-label')).toHaveValue('Example: Country codes')
})
