...
test('an extension-contributed secret source lists its keys by name and an entry can point at one', async () => {
	...
	try {
		...
		const storedHere = page.getByTestId('secret-storage-here')
		const fromSource = page.getByTestId('secret-storage-source')
		await expect(storedHere).toHaveAttribute('aria-pressed', 'true')
		await expect(fromSource).toHaveAttribute('aria-pressed', 'false')

		await fromSource.click()
		await expect(fromSource).toHaveAttribute('aria-pressed', 'true')
		await expect(storedHere).toHaveAttribute('aria-pressed', 'false')
		await expect(page.getByTestId('secret-source-select')).toBeVisible()

		await storedHere.click()
		await expect(storedHere).toHaveAttribute('aria-pressed', 'true')
		await expect(fromSource).toHaveAttribute('aria-pressed', 'false')
		await expect(page.getByTestId('secret-source-select')).toHaveCount(0)
		await expect(page.getByTestId('secret-password-input')).toBeVisible()
