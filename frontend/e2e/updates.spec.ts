...
test('Source-channel build never offers to download, and shows the pull-and-rebuild instructions', async ({}, testInfo) => {
  ...
  try {
    ...
    // The changelog surface itself (goal 0220 S2) is proven end-to-end
    // by its own dedicated tests below -- this one only needs the raw
    // details/pre-wrap rendering to be gone from the card.
    await expect(card.getByText("What's new")).toHaveCount(0)
...
// The changelog surface (goal 0220 S2), command-first (0222): before
// any check has ever run, update.whatsNew is still reachable via the
// palette (no Settings visit, no pill) and shows the honest empty
// state with its own action beside it, never a dead-end sentence
// (ux-writing.md's "an empty state offers the action it names"). No
// MILL_TEST_UPDATE_FAKE_VERSION here on purpose -- opening Settings
// would fire a real (offline-failing) check via its own auto-check-
// on-open effect, so this test never navigates there.
// eslint-disable-next-line no-empty-pattern -- this test needs `testInfo` (the second arg), not any fixture.
...
test("The pill's secondary link and Settings' own link both open What's new, rendering the notes as real markdown", async ({}, testInfo) => {
  ...
  try {
    ...
    // runs the fake check here -- the same path every other test in
    // this file relies on to light the pill.
    await page.getByRole('link', { name: 'Settings' }).click()
    await expect(page.getByTestId('update-available-card')).toBeVisible()
