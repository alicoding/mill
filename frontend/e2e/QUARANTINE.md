...
| atlas-slots.spec.ts:59 | interaction-race (midpoint chip intercepts the default hover point) → FIXED 2026-09-06 (goal 0358 S2) | 2026-09-01 | — | second same-day sighting, BOTH failing all retries under CI shard-6 load (main run 07:35Z after #550's merge: toHaveCount mismatch; PR #558's run 19:45Z). Not reproduced locally; traces auto-captured on retry -- read the newest trace before theorizing. Register-only entry: retry-passing is not a fix and neither sighting recurred on a rerun of the same commit. THIRD sighting 2026-09-02 on PR #569's shard 6 (flaky: failed first attempt, passed on retry), FOURTH the same morning on PR #571's shard 6 failing all three attempts (a rerun of the shard passed -- the change under test never touched linking) alongside first-ever sightings of atlas-linking.spec.ts:27 and atlas-single-space-trap.spec.ts:34 in the same shard -- a shard-6 load cluster; those two stay unregistered until a second strike. FIFTH 2026-09-06 05:19-05:22Z on shard 9 (job 101432768216, run 34013196673): 90s test timeout, all 3 attempts. Read the trace per the standing instruction: the quiet-edges phase's bare `edge.hover()` hung -- 83 actionability retries, every one intercepted by the visible chip's edge-change-kind button, plus occasional edge detaches. Measured locally (reproduced 2/6 throttled): the chip renders at the edge midpoint + 18 FLOW units (AtlasLinkEdge), inside the zoomed viewport, so under zoomAllTheWayOut it shrinks onto the line's midpoint -- exactly the bounding-box center Playwright's default hover targets -- and a stuck-hovered edge (the dropped-mouseleave class this file already documents) leaves the chip visible to intercept there forever. Fix: the spec hovers/right-clicks a quarter along the interaction path (edgePositionOffChip), the same gesture off the chip. 3/3 + 3/3 under 4x throttle, 3/3 unthrottled. Product-side observation handed to the orchestrator: at zoom ≲0.8 the chip's scaled-down buttons overlap the interaction path itself, so a midpoint click/hover on a hovered edge hits the chip, not the edge |
...
| decision-outcome.spec.ts:142 | local-only-red (env divergence) | 2026-09-04 | 2026-10-04 | SECOND strike: `dblClickReachedTerminal` cannot reach the terminal at this display on origin/main (first: 0326 builder's local run; confirmed in a throwaway worktree); passes in CI. Chase under `E2E_CPU_THROTTLE=4` only if it ever fails in CI |
...
| atlas-folder-import.spec.ts:33 | lazy-engine-wait → FIXED 2026-09-02, RECURRED → FIXED 2026-09-06 (goal 0358 S2) | 2026-09-02 | — | three CI sightings in one day (the mermaid diagram never counted in the overlay); instrumented under 4x throttle: the render completed AFTER the default 5s expect expired (the engine is a lazy chunk loaded once the prose mounts). Assertions now carry 30s; 16/16 at 4x. Class note: any assertion on a lazily-loaded engine's output (mermaid, draw.io, pdf, CodeMirror) needs engine headroom, not the default wait. SECOND STRIKE after the fix: recurred 2026-09-05 17:21Z on #710's CI and in the owner's main-run measurement the same day -- the 30s count timeout could not tell "still loading" from "never will render". Measured under 4x throttle with a MutationObserver + per-run markers: the mirror div's whole child set (rm8 add8) was re-assigned MID-RENDER by React itself -- dangerouslySetInnerHTML's `{__html}` wrapper is an inline literal compared by REFERENCE, so every unrelated parent re-render rewrote innerHTML, wiping the rendered wrapper and stranding the fences the hook captured (the window widens with chunk load time); a rejected lazy-chunk import was additionally memoized forever. Fix: the markdown mirror renders behind a memo boundary (MirrorMarkdownHost, the MermaidDiagramHost shape), the host publishes data-mermaid-state pending→settled (plus data-mermaid-error), a rejected chunk import clears its own cache, and the spec waits on the settled signal. 6/6 at 4x throttle |
...
| atlas-shape-tool.spec.ts (file-level) | shared-server-test-coupling → FIXED 2026-09-06 (goal 0358 S2) | 2026-09-05 | — | "the shape style choice survives a disarm/re-arm cycle" failed on main runs where the file ran in ONE worker -- EVERY completed draw persists {type, stroke, width} to the mill-drawing plugin's 'shape' storage key, which lives on the shared worker server and survives each test's fresh page: the file's own third test leaves type=arrow, so the fourth test's draw lands an ARROW whose promote popover titles itself 'Arrow', not 'Rectangle' (and an earlier shape spec on a multi-file shard can likewise rewrite the shipped-default assertions). Reproduced deterministically in-order locally. Fix: the spec's beforeEach deletes the 'shape' key through SettingsService.DeletePluginStorageValue over the runtime wire before the page loads the plugin. 7/7 unthrottled, and the failing test 3/3 under 4x throttle |
| atlas-shape-tool.spec.ts:57 | unclear (app crash, React error #185) | 2026-09-06 | 2026-10-06 | FIRST class of sighting: under 4x CPU throttle the page hit the app error boundary with minified React error #185 (maximum update depth) from React Flow's StoreUpdater -> setNodes -> forceStoreRerender loop, AtlasBoardInner in the component stack, during the FIRST test's initial board mount -- 2/3 repeats red in one throttled file run on the 0358-S2 branch (merged main 4b4a49a3), then 6/6 green on isolated repeats of the same test. App-side crash, not a test assertion: no test code had run beyond goto+arrive at the board; the branch's own diffs only touch the card overlay's mirror preview, never the board's node array. Read the error-context screenshot/stack in test-results before theorizing; if it recurs on main, suspect a render loop between the Board menu chrome (#726) state and React Flow's store sync before any other theory |
...
|---|---|---|---|
| docs-view.spec.ts:137 (openDocsSearch) | shortcut-first-before-mount | 2026-09-04 | shortcut pressed before the app's plugin-gated first render; fixed by `gotoAppReady`. Was misfiled as local-only-red (env divergence): the real mechanism is main.tsx's `bootstrap()` rendering `<App/>` only after an async plugin-load-gated chain, so a `page.goto('/')` resolving races ahead of `useKeymapDispatch`'s window keydown listener attaching -- `openDocsSearch`'s Meta+K was the first action on the page. Same class fixed in keymap.spec.ts:205 (Cmd+/) and help-overlay.spec.ts:47 (Cmd+Shift+/), all migrated to `gotoAppReady`/`waitForAppReady` (fixtures/appReady.ts), which waits for App.tsx's `data-app-ready` marker before the first shortcut |
