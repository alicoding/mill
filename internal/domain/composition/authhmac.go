...
// using the connector's keychain secret as the key, sent as
...
// future work, §3.2's own "extend when a real connector needs it"
// principle applied here the same as everywhere else.
func init() {
	RegisterAuthStrategy(connector.AuthHMAC, func(rc ResolvedConnector, method, path string, headers map[string]string, query url.Values, body string) error {
