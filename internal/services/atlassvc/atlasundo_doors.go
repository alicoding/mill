...
var journaledDoors = map[string]string{
	...
	"CreateFileObjectFromDownload": "create family; composes CreateBoardObject's own recordUndo call " +
		"(a dedupe hit creates nothing new, so there is nothing for undo to record then)",
	...
	"SetBoardObjectKind":     "scalar family",
...
var exemptDoors = map[string]string{
	...
	"SaveFileBytes":                       "writes an arbitrary file to disk; the resulting board entity lands via a separate, already-journaled create door (same split as SaveImageBytes)",
	...
	"ReconcilePluginCanvasObjectExamples": "wails:ignore, boot-time seed reconcile (goal 0411), not a single user gesture -- same class as reconcileBuiltIns' own construction-time pass, which this file's own doors list has never journaled",
...
var notMutationDoors = map[string]string{
	...
	"PickBoardFile": "native file picker, no board-state change",
