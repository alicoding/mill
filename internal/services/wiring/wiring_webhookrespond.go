...
// neither service package imports the other (bridgeservice_webhook.go's
...
// WireWebhookRespond arms the webhook door to wait for a
// respond-webhook reply: DispatchWebhookEvent's own
// WebhookWait/WebhookReply shape is relayed onto bridgesvc's
// identically-shaped types, one goroutine per post, so a caller's one
// HTTP response can be answered by a run this package didn't start.
