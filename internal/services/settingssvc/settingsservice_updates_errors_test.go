...
func TestClassifyUpdateFailureStage(t *testing.T) {
	cases := []struct {
		...
		{"missing computed digest", errors.New("updater: no digest available; download path bug"), UpdateFailureStageInstall},
		{"unknown digest algorithm", errors.New(`updater: unknown digest algorithm "sha999"`), UpdateFailureStageInstall},
		{"unsupported signature algorithm", errors.New(`updater: unsupported signature algorithm "rsa"`), UpdateFailureStageInstall},
		{"signature metadata incomplete", errors.New("updater: signature present but signatureAlgo missing"), UpdateFailureStageInstall},
