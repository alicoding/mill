...
// framework-api-audit: wails/v3@v3.0.0-beta.18 lacks a way to read SMAppServiceStatusRequiresApproval -- App.Autostart (pkg/application/autostart_manager.go, backed by SMAppService on macOS 13+ per autostart_darwin_smappservice.go) covers Enable/Disable/IsEnabled/Status, but Status() collapses that one pending-approval bit away.
