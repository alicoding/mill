...
func (s *Store) List(filter Filter, limit, offset int) ([]secretaudit.Record, int, error) {
	...
	q := "SELECT id, timestamp, entry_id, label, context, run_id, workflow_id, actor, outcome, error_text, step_id FROM secret_access " +
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &ts, &r.EntryID, &r.Label, &ctxVal, &r.RunID, &r.WorkflowID, &r.Actor, &outcome, &r.ErrorText, &r.StepID); err != nil {
